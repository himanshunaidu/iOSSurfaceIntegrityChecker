...
struct ARCroppedViewControllerContainer: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> ARCroppedHostViewController {
        let vc = ARCroppedHostViewController()
    ...
    func updateUIViewController(_ uiViewController: ARCroppedHostViewController, context: Context) {
...
final class ARCroppedHostViewController: UIViewController, ARSessionDelegate {
    ...
    private let imageBoundingBox = ImageBoundingBox()
    // MARK: - Processing
...
//        view.translatesAutoresizingMaskIntoConstraints = false
        ...
        aspectConstraint = view.widthAnchor.constraint(equalTo: view.heightAnchor, multiplier: 3/4)
...
//            arView.centerYAnchor.constraint(equalTo: view.centerYAnchor)
        ...
        arView.addSubview(segmentationFrameImageView)
        applySegmentationFrameLayoutIfNeeded()
...
//            overlayImageView.centerYAnchor.constraint(equalTo: view.centerYAnchor)
    ...
    // MARK: - Layout / debug
    func applySegmentationFrameLayoutIfNeeded() {
        NSLayoutConstraint.deactivate(segmentationFrameImageView.constraints)
        NSLayoutConstraint.activate([
            segmentationFrameImageView.topAnchor.constraint(equalTo: view.topAnchor),
            segmentationFrameImageView.bottomAnchor.constraint(equalTo: view.bottomAnchor),
            segmentationFrameImageView.leadingAnchor.constraint(equalTo: view.leadingAnchor),
            segmentationFrameImageView.trailingAnchor.constraint(equalTo: view.trailingAnchor)
        ])
    }
...
//            damageOverlayImageView.centerYAnchor.constraint(equalTo: view.centerYAnchor)
        ])
    }
...
//        print("Unique values in rendered mask: \(uniqueValues.sorted())")
    ...
    private func processOverlay(pixelBuffer: CVPixelBuffer, exifOrientation: CGImagePropertyOrientation, cameraTransform: simd_float4x4, cameraIntrinsics: simd_float3x3) {
        autoreleasepool {
            ...
            let segmentationFrameImage = imageBoundingBox.create(imageSize: originalSize, boundingBoxSize: croppedSize, postOrientation: exifOrientation)
...
//            segmentationColor = segmentationColor.transformed(by: CGImagePropertyOrientation.right.toUpTransform(for: segmentationColor.extent.size))
            ...
            let damageDetectionResults = damageDetectionFrameProcessor.processRequest(with: pixelBuffer, orientation: exifOrientation.inverted)
...
//            }
            ...
            DispatchQueue.main.async { [weak self] in
                self?.segmentationFrameImageView.image = segmentationFrameImage
