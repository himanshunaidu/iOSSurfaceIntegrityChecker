...
enum SegmentationPipelineError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unexpectedError:
            return "An unexpected error occurred in the SegmentationPipeline."
        }
    }
}
...
class SegmentationPipeline: ObservableObject {
    ...
    func processRequest(with cIImage: CIImage, previousImage: CIImage?, deviceOrientation: UIDeviceOrientation = .portrait,
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            } catch let error as SegmentationPipelineError {
                DispatchQueue.main.async {
                    self.completionHandler?(.failure(error))
                }
            } catch {
                DispatchQueue.main.async {
                    self.completionHandler?(.failure(SegmentationPipelineError.unexpectedError))
                }
            }
            self.isProcessing = false
        }
    ...
    /**
     ...
     It will either return the SegmentationPipelineResults or throw an error.
    ...
    func processImage(_ cIImage: CIImage, previousImage: CIImage? = nil,
                      deviceOrientation: UIDeviceOrientation = .portrait) throws -> SegmentationPipelineResults {
        ...
        guard let segmentationImage = segmentationResults?.segmentationImage else {
            throw SegmentationPipelineError.invalidSegmentation
        }
        ...
        return SegmentationPipelineResults(
