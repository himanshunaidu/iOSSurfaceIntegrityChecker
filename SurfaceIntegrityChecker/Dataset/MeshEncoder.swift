...
class MeshEncoder {
    ...
    func save(meshBundle: MeshBundle, frameNumber: UUID) {
        ...
        do {
            ...
            let ply = generatePlyContent([greenPly, redPly], includeColor: true)
    ...
    func generatePlyContent(
        ...
        if includeColor {
            for content in plyContents {
                for pos in content.positions {
                    ply += "\(pos.x) \(pos.y) \(pos.z) \(content.colorR8) \(content.colorG8) \(content.colorB8)\n"
                }
            }
        } else {
            for content in plyContents {
        ...
        for content in plyContents {
            ...
            for f in stride(from: 0, to: faceCount, by: 3) {
